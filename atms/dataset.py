...
class MyDataset(data.Dataset):
	...
	def __init__(self, split, img_dir, img_dir_dev, vocab, transform, what_elements='triplet', load_image_feature=0,**kwargs):
		"""
		Args:
		...
		self.img_dir_dev = img_dir_dev
		self.vocab = vocab
		self.transform = transform
		self.what_elements = what_elements
		self.find_get_item_func()
		# self.data_name = kwargs["opt"].data_name
	...
	def get_transformed_image_dev(self, path):
		image = Image.open(os.path.join(self.img_dir_dev, path)).convert('RGB')
		# transform the image (normalization & resizing + data augmentation)
		image = self.transform(image)
		return image
