...
from dataset_amazon import AmazonDataset
...
def get_loader_single(opt, vocab, split, transform, what_elements="triplet",
	...
	if opt.data_name == 'fashionIQ':
		dataset = FashionIQDataset(split, vocab, transform, what_elements,opt.load_image_feature, fashion_categories=opt.categories)
	...
	elif opt.data_name == 'amazon':
		dataset = AmazonDataset(split,vocab,transform,what_elements,opt.load_image_feature)
	# Data loader
	# pdb.set_trace()
	if opt.parallel:
...
def get_eval_loader_generic(opt, vocab, split, what_elements,drop_last):
	transform = get_transform(opt, phase="eval")
	loader = get_loader_single(opt, vocab, split, transform, what_elements, # what_element = query or target
									shuffle=False, drop_last=drop_last)
	return loader

# Functions to be called from outside this file

def get_search_loader(opt, vocab, shuffle=True):
	transform = get_transform(opt, phase="search")
	triplet_loader_train = get_loader_single(opt, vocab, "train", transform, "triplet",
											shuffle=shuffle, drop_last=True)
	triplet_loader_dev = get_loader_single(opt, vocab, "dev", transform, "triplet",
									   shuffle=shuffle, drop_last=True)
	triplet_loader_test = get_loader_single(opt, vocab, "test", transform, "triplet",
										   shuffle=False, drop_last=True)
	return triplet_loader_train,triplet_loader_dev,triplet_loader_test

def get_train_loader(opt: object, vocab: object, split: object = 'train', shuffle: object = True) -> object:
...
def get_eval_loaders(opt, vocab, split='val'):
	queries_loader = get_eval_loader_generic(opt, vocab, split, "query",drop_last = True)
	targets_loader = get_eval_loader_generic(opt, vocab, split, "target",drop_last = False)
...
def collate_fn_query(data):
	...
	# Sort a data list by sentence length
	data.sort(key=lambda x: len(x[1]), reverse=True)
	# images_src, sentences, img_src_ids, img_trg_ids, raw_caps, dataset_ids,img_src_pids,img_trg_pids = zip(*data)
	images_src, sentences, images_trg,img_src_ids, img_trg_ids, raw_caps, dataset_ids = zip(*data)
	# Merge images (convert tuple of 3D tensor to 4D tensor)
	images_src = torch.stack(images_src, 0)
	images_trg = torch.stack(images_trg, 0)
	# Merge sentences (convert tuple of 1D tensor to 2D tensor)
	...
	return images_src, sentences_padded, lengths, img_src_ids, img_trg_ids, raw_caps, dataset_ids,images_trg


def collate_fn_query_amazon(data):
	"""Build mini-batch tensors from a list of tuples.
	Args:
		data: list of (image_src, sentence, img_src_id, img_trg_id, raw_caption, dataset_index) tuple.
			- image_src : torch tensor of shape (3, 256, 256) or (*).
	...
	return images_src, sentences_padded, lengths, img_src_ids, img_trg_ids, raw_caps, dataset_ids,img_src_pids,img_trg_pids
...
def get_train_loaders(args, vocab):
	return None