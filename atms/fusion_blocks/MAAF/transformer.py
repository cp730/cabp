...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, concat_feat, mask):
        "Take in and process masked sequence."
        return self.encoder(self.pos_embed(concat_feat), mask)
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, N):
        ...
        self.layers = clones(layer, N) # 复制N个self attention+FFN+ residual的层
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout):
        ...
        self.sublayer = clones(SublayerConnection(size, dropout), 2) # 产生2个SublayerConnection层
        self.size = size

    def forward(self, x, mask):
        "Follow Figure 1 (left) for connections."
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask)) # q,k,v，self attention + residual  connection
        return self.sublayer[1](x, self.feed_forward) # FFN+residual connection
