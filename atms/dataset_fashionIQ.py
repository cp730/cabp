...
class FashionIQDataset(MyDataset):
    ...
    def __init__(self, split, vocab, transform, what_elements='triplet', load_image_feature=0,
        ...
        """
        MyDataset.__init__(self, split, FASHIONIQ_IMAGE_DIR, FASHIONIQ_ANNOTATION_DIR, vocab, transform=transform,
                           what_elements=what_elements, load_image_feature=load_image_feature, **kw)
        self.split = split
        fashion_categories = ['dress', 'shirt', 'toptee'] if fashion_categories == 'all' else sorted(
            fashion_categories.split())

        image_id2name_files = [os.path.join(FASHIONIQ_ANNOTATION_DIR, 'image_splits', f'split.{fc}.{split}.json') for fc
                               in fashion_categories]
        ...
        if self.what_elements in ["query", "triplet"]:
            prefix = 'cap'
            annotations_files = [os.path.join(FASHIONIQ_ANNOTATION_DIR, 'captions', f'{prefix}.{fc}.{split}.json') for
                                 fc in fashion_categories]
    ...
    def get_triplet(self, idx):
        ...
        text, real_text = self.get_transformed_captions(
            capts)  # text: tensor;real_text==='has purple color with text <and> is more colorful'

        path_src = ann['candidate'] + ".png"
        img_src = self.get_transformed_image(path_src)  # torch.Size([3, 224, 224])

        path_trg = ann['target'] + ".png"
        img_trg = self.get_transformed_image(path_trg)  # torch.Size([3, 224, 224])
    ...
    def get_query(self, idx):
        ...
        return img_src, text, img_trg, img_src_id, img_trg_id, real_text, idx
