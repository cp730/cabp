...
from config import MAIN_DIR, FASHIONIQ_ANNOTATION_DIR, SHOES_ANNOTATION_DIR, CIRR_ANNOTATION_DIR, FASHION200K_ANNOTATION_DIR, cleanCaption,AMAZON_ANNOTATION_DIR
...
ANNOTATIONS = {
  ...
  'amazon':[f'{AMAZON_ANNOTATION_DIR}/apparel_train_annotation.csv']
}
...
################################################################################
def from_amazon_json(p):
  csv_data = pd.read_csv(p,encoding='gb18030')

  feedbacks1 = csv_data['Feedback 1'].tolist()
  feedbacks2 = csv_data['Feedback 2'].tolist()
  feedbacks3 = csv_data['Feedback 3'].tolist()
  captions = [cleanCaption(a) for a in feedbacks1]
  captions += [cleanCaption(a) for a in feedbacks2]
  captions += [cleanCaption(a) for a in feedbacks3]
  return captions
...
if __name__ == '__main__':
  ...
  parser.add_argument('--data_name', default='fashionIQ', choices=('fashionIQ', 'shoes', 'cirr', 'fashion200K','amazon'), help='Name of the dataset for which to build the vocab (fashionIQ|shoes|cirr|fashion200K)')
