...
#codepath="/home/lihangfei/projects/artemis-main/"
...
RUN=ce_loss_noise_wo_tscale # if you want to train the same model several times, use this as a form of identifier
...
#        --validate "test-val" # comment this line if you do not have the test split annotations
    ...
    specific_args=(
        --load_image_feature 2048
        # loaded feature size (usually: 0)
        --cnn_type 'resnet152'
    )
...
###
    python3 ${codepath}/train.py "${exp_args[@]}" "${specifi0c_args[@]}" \
    --num_epochs 8 --step_lr 4 --learn_temperature --pretrain

    python3 ${codepath}/train.py "${exp_args[@]}" "${specific_args[@]}" \
    --img_finetune --txt_finetune --learn_temperature --lr 3.90625e-06 \
    --ckpt ${CKPT_DIR}/${exp_name}/ckpt.pth

elif  [ "$PHASE" == "search" ] ; then
   ...
   if [ "$DATASET" == "fashionIQ" ] ; then
    ...
    python3 ${codepath}/train.py "${exp_args[@]}" "${specific_args[@]}" \
     --learn_temperature --img_finetune --txt_finetune --num_epochs 40
