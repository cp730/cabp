...
class FusionCell(nn.Module):
    '''
     fusion cell：fusion cell内部有多个cell组成
    '''
    def __init__(self, steps, multiplier, args):
        ...
        self._step_nodes = nn.ModuleList() # 一个cell中的step结点列表
        self.num_input_nodes = args.num_input_nodes # total number of modality features
        self.C = args.C # channels for conv layer
        self.L = args.L # length after conv and pool
        self.ln = nn.LayerNorm([self.C * self._multiplier, self.L])

        # input features is a joint list of visual_features and skel_features
        # for each cell
        for i in range(self._steps):  # step == cell
            for j in range(self.num_input_nodes+i): # 每一个cell默认都会和它前面所有的结点的输出相连---即在upper level中一共有多少条边，就有多少个操作要初始化
    ...
    def _initialize_step_nodes(self, args):
        for i in range(self._steps): # for each cell
...
class FusionNetwork(nn.Module):
    '''
    fusion network:将两个模态的不同层次的特征输入到fusion cell中，
    fusion cell 
    '''
    def __init__(self, steps, multiplier, num_input_nodes, num_keep_edges, args, 
        ...
        self.logger = logger
        self._steps = steps
        self._multiplier = multiplier # cell output concat default = 2 
        self._criterion = criterion

        # input node number in a cell
        self._num_input_nodes = num_input_nodes # 一共有多少个特征用到，default = 6
        self._num_keep_edges = num_keep_edges # 每个cell和step有多少个输入。default = 2 
